...
static char* coy_function_read_symbol_(struct coy_function_* func, struct coy_memio_* memio, uint32_t pos, uint32_t len)
{
...
static bool coy_function_read_consts_(struct coy_function_* func, struct coy_memio_* memio)
{
...
static bool coy_function_read_blocks_(struct coy_function_* func, struct coy_memio_* memio)
{
    ...
    {
        struct coy_function_block_* block = &func->u.coy.blocks[b];
...
static bool coy_function_read_instrs_(struct coy_function_* func, struct coy_memio_* memio)
{
...
static void coy_function_compute_maxslots_(struct coy_function_* func)
{
    uint32_t maxslots = 0;
...
static bool coy_function_verify_jmp_block_(struct coy_function_* func, const struct coy_function_block_* block, const union coy_instruction_* instr, uint32_t i, coy_bitarray_t isptr, coy_bitarray_t jmpisptr, uint32_t jmpb, uint32_t ahead, uint32_t atail)
{
    uint32_t anum = atail - ahead;
    COY_VERIFY_(jmpb < stbds_arrlenu(func->u.coy.blocks), "jmp block index out of range");
    const struct coy_function_block_* jmpblock = &func->u.coy.blocks[jmpb];
...
static bool coy_function_verify_numeric_args_(struct coy_function_* func, const struct coy_function_block_* block, const union coy_instruction_* instr, uint32_t i, coy_bitarray_t isptr, coy_bitarray_t jmpisptr)
{
    for(uint32_t a = 0; a < instr->op.nargs; a++)
        COY_VERIFY_REGVALA_(a);
    return true;
}
static bool coy_function_verify_jmp_(struct coy_function_* func, const struct coy_function_block_* block, const union coy_instruction_* instr, uint32_t i, coy_bitarray_t isptr, coy_bitarray_t jmpisptr)
{
    return coy_function_verify_jmp_block_(func, block, instr, i, isptr, jmpisptr, instr[1].raw, 1, instr->op.nargs);
}
static bool coy_function_verify_jmpc_(struct coy_function_* func, const struct coy_function_block_* block, const union coy_instruction_* instr, uint32_t i, coy_bitarray_t isptr, coy_bitarray_t jmpisptr)
{
...
typedef bool coy_function_verify_helper_(struct coy_function_* func, const struct coy_function_block_* block, const union coy_instruction_* instr, uint32_t i, coy_bitarray_t isptr, coy_bitarray_t jmpisptr);
...
static bool coy_function_coy_verify_(struct coy_function_* func)
{
    ...
    for(uint32_t b = 0; b < nblocks; b++)
    {
        const struct coy_function_block_* block = &func->u.coy.blocks[b];
